import React from 'react';
import { NavLink, Outlet } from 'react-router-dom';
import { useAuthContext } from '../../context/AuthContext';
import { PATH } from '../../constant/common';

const StudentProfilePage = () => {
  const { profile } = useAuthContext();
  const { profileImage, firstName: name, email, introduce, phone, website, facebookURL: facebook } = profile || {};

  return (
    <main className="mainwrapper profilepage">
      <div className="container">
        <div className="wrapper">
          <div className="sidebar">
            <div className="sidebar__info">
              <div className="useravatar">
                <div className="avatar">
                  <div className="img">
                    <img src={profileImage || '/img/cfd-share-thumbnail-facebook.png'} alt="avatar" />
                  </div>
                </div>
                <h3 className="title --t3">{name}</h3>
              </div>
            </div>
            <div className="sidebar__content">
              <h4>Giới thiệu</h4>
              {!!introduce && <p className="description">{introduce}</p>}
              <ul>
                <li>
                  <img src="/img/icon-mail-outline.svg" alt="icon" />
                  <span>{email}</span>
                </li>
                {!!phone && (
                  <li>
                    <img src="/img/icon-phone-outline.svg" alt="icon" />
                    <span>{phone}</span>
                  </li>
                )}
                {!!website && (
                  <li>
                    <img src="/img/icon-link.svg" alt="icon" />
                    <a href={website} target="_blank">
                      {website}
                    </a>
                  </li>
                )}
              </ul>
              <div className="social">
                {!!facebook && (
                  <a href={facebook} target="_blank">
                    <img src="/img/icon-fb-footer.svg" />
                  </a>
                )}
              </div>
            </div>
          </div>
          <div className="tabwrap">
            <div className="tab">
              <div className="tab__title">
                <NavLink end to={PATH.PROFILE.INDEX}>
                  Thông tin cá nhân
                </NavLink>
                <NavLink to={PATH.PROFILE.MY_COURSES}>Khóa học của tôi</NavLink>
                <NavLink to={PATH.PROFILE.MY_PAYMENT}>Lịch sử thanh toán</NavLink>
              </div>
              <div className="tab__content">
                <Outlet />
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>
  );
};

export default StudentProfilePage;
